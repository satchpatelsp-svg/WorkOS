...
export const CATALOG_QUERIES = Object.freeze({
  ...
  ledgerPrimaryKey: `
    select attribute.attname as column_name,
      constraint.contype = 'p' as is_primary_key
    from pg_constraint constraint
    join pg_class relation on relation.oid = constraint.conrelid
    join pg_namespace namespace on namespace.oid = relation.relnamespace
    join unnest(constraint.conkey) with ordinality key(attnum, ordinal) on true
    join pg_attribute attribute on attribute.attrelid = relation.oid and attribute.attnum = key.attnum
    where namespace.nspname = 'public' and relation.relname = 'workos_migrations'
